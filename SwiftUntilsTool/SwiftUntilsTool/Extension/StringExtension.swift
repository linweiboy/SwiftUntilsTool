...
//  RongTeng
//
//  Created by rongteng on 16/4/25.
...
extension String {
  ...
  public var cgFloatValue:CGFloat? {
    return CGFloat(NumberFormatter().number(from: self)?.floatValue ?? 0)
  }
...
extension String {
  ...
  ///获取宽度
  public func getWidth(maxWidth:CGFloat,font:CGFloat) -> CGFloat {
    if #available(iOS 10, *) {
      return achieveStringRect(maxWidth, font: font).width + 3
    }
    return achieveStringRect(maxWidth, font: font).width
  }
  
  ///获取高度
  public func getHeight(maxWidth:CGFloat,font:CGFloat) -> CGFloat {
    return achieveStringRect(maxWidth, font: font).height
  }
